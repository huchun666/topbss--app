...
@Component({
  selector: 'page-order-list-pay',
  templateUrl: 'order-list-pay.html',
})
export class OrderListPayPage {
  ...
  getOrderList() {
    ...
    var url = `${AppConfig.API.getOrderList}?userType=B&status=0&start=${this.start}&limit=${this.pageSize}`;
    if (this.paramsDate != '')
      url += this.paramsDate;
  ...
  loadMore(infiniteScroll) {
    let url = `${AppConfig.API.getOrderList}?userType=B&status=0&start=${this.start}&limit=${this.pageSize}`;
    if (this.paramsDate != '')
      url += this.paramsDate;
    this.appService.httpGet(url).then(data => {
      if (this.start < data.count) {

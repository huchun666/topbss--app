...
@Component({
  selector: 'page-brandshop-order-finished',
  templateUrl: 'brandshop-order-finished.html',
})
export class BrandshopOrderFinishedPage {
  ...
  getOrderList() {
    ...
    var url = `${AppConfig.API.getOrderList}?status=C&start=${this.start}&limit=${this.pageSize}`;
    if (this.paramsDate != '')
      url += this.paramsDate;

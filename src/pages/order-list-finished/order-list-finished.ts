...
export class OrderListFinishedPage {
  ...
  getOrderList() {
    ...
    var url = `${AppConfig.API.getOrderList}?userType=B&status=C&start=${this.start}&limit=${this.pageSize}`;
    if (this.paramsDate != '')
      url += this.paramsDate;
  ...
  loadMore(infiniteScroll) {
    let url = `${AppConfig.API.getOrderList}?userType=B&status=C&start=${this.start}&limit=${this.pageSize}`;
    if (this.paramsDate != '')
      url += this.paramsDate;
    this.appService.httpGet(url).then(data => {
      if (this.start < data.count) {
